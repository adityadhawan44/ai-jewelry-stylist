...
export default async function handler(req, res) {
  ...
  const { imageDataUrl, imageMeta, context } = body
  ...
  if (!process.env.OPENAI_API_KEY) {
    return res.status(200).json({
      ...
      usedAi: false,
      narrative:
        'No OpenAI key is configured on Vercel yet, so the app is using the local stylist engine instead of server-side vision analysis.',
    })
  }
...
${JSON.stringify(imageMeta || {}, null, 2)}
    ...
    return res.status(500).json({
      ...
      narrative:
        'The AI request failed on the hosted API, so the app switched to the local stylist engine and still produced a polished result.',
    })
  }
}
